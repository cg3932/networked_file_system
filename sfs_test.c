  ...
  for (i = 0; i < nopen; i++) {
    ...
    if (fds[i] >= 0) {
      ...
      if (readsize != strlen(test_str)) {
        fprintf(stderr, "ERROR: Read wrong number of bytes %d!=%d(%d)\n", readsize, strlen(test_str), sizeof(fixedbuf));
        error_count++;
      }
...
main2(int argc, char **argv)
{
  ...
  mksfs(1);                     /* Initialize the file system. */
  sfs_ls();
  int fd = sfs_open("testing.123");
  printf("write=%d\n", sfs_write(fd, buf2, sizeof(buf2)));
  sfs_close(fd);
  fd = sfs_open("testing.123");
  printf("read=%d\n", sfs_read(fd, buf2, sizeof(buf2)));
  printf("%s", buf2);
  printf("\n");
  sfs_close(fd);
  mksfs(0);                     /* Initialize the file system. */
